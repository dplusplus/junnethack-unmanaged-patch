    ...
    xchar x, y;
{
	...
	You("start chewing %s %s.",
	    ...
	    You("%s%sはじめた。",
		boulder ? "岩" :
		IS_TREES(lev->typ) ? "木" : IS_ROCK(lev->typ) ? "石" : "扉",
		boulder ? "を噛み" : "に穴をあけ");
#endif
...
*/
	 if (mode == DO_MOVE) autoexplore_msg("何か", mode);
	 return FALSE;
    }
...
*/
	    if (sobj_at(BOULDER,x,y) && mode == DO_MOVE) autoexplore_msg("岩", mode);
	    return FALSE;
	}
	if (mode == DO_MOVE) {
            ...
            if (iflags.autoexplore) {
                ...
                if (In_sokoban(&u.uz)) {
                  ...
                  pline("But, you feel you should be careful moving"
                        " around.");
#else
                  pline("探索しなくてもあなたはこの辺りの地理には"
                        "通じている。");
                  pline("しかしながら、あなたは慎重に行動すべきだと"
                        "感じた。");
#endif
                  flags.move = 0;
                  nomul(0, 0);
...
*/
					Sprintf(msg, "ある程度探索したが、%d箇所行けない場所がある。", unexplored_cnt);
				} else {
/*JP
					Strcpy(msg, "Partly explored, can't reach some places.");
*/
					Strcpy(msg, "ある程度探索したが、何箇所か行けない場所がある。");
				}
			} else {
/*JP
...
*/
			You("%s%sを感じた。", buf, strange_vibration);
