...
*/
	Sprintf(qbuf,"Ç±ÇÃäKëwÇâΩÇ∆åƒÇ—Ç‹Ç∑Ç©ÅH");
	getlin(qbuf, nbuf);
...
/*JP
	case 2: return "some";
*/
	case 2: return "Ç¢Ç≠Ç¬Ç©ÇÃ";
/*JP
	case 3: return "many";
*/
	case 3: return "ëÚéRÇÃ";
	}
...
*/
	case BR_PORTAL:	 return closed_portal ? "ïïàÛÇ≥ÇÍÇΩñÇñ@ÇÃì¸å˚" : "ñÇñ@ÇÃì¸å˚";
/*JP
	case BR_NO_END1: return "Connection";
*/
	case BR_NO_END1: return "ï™äÚì_";
#if 0 /*JP*/
	case BR_NO_END2: return (br->end1_up) ? "One way stairs up" : 
		"One way stairs down";
#else
	case BR_NO_END2: return (br->end1_up) ? "ï–ìπÇÃè„ÇËäKíi" : 
		"ï–ìπÇÃâ∫ÇËäKíi";
#endif
/*JP
	case BR_STAIR:	 return (br->end1_up) ? "Stairs up" : "Stairs down";
*/
	case BR_STAIR:	 return (br->end1_up) ? "è„ÇËäKíi" : "â∫ÇËäKíi";
	}
...
*/
			Sprintf(buf, "%s:", jtrns_obj('d', dungeons[mptr->lev.dnum].dname));
		else
#if 0 /*JP*/
...
#else
			Sprintf(buf, "%s: %däKÇ©ÇÁ%däK", 
				jtrns_obj('d', dungeons[mptr->lev.dnum].dname),
				depthstart, depthstart + 
				dungeons[mptr->lev.dnum].dunlev_ureached - 1);
#endif
		if (!dump) {
...
*/
		Sprintf(buf, TAB "ëÊ%iäE:", -i);
	else
/*JP
		Sprintf(buf, TAB "Level %d:", i);
*/
		Sprintf(buf, TAB "%däK:", i);
...
*/
			ADDNTOBUF("ç’íd", mptr->feat.naltar)
		else
/*JP
			ADDNTOBUF("temple", mptr->feat.ntemple)
*/
			ADDNTOBUF("éõâ@", mptr->feat.ntemple)
...
*/
		ADDNTOBUF("êÚ", mptr->feat.nfount)
/*JP
		ADDNTOBUF("sink", mptr->feat.nsink)
*/
		ADDNTOBUF("ó¨Çµë‰", mptr->feat.nsink)
/*JP
		ADDNTOBUF("throne", mptr->feat.nthrone)
*/
		ADDNTOBUF("ã ç¿", mptr->feat.nthrone)
/*JP
		ADDNTOBUF("tree", mptr->feat.ntree);
*/
		ADDNTOBUF("ñÿ", mptr->feat.ntree);
		/*
...
#else
		Sprintf(buf, PREFIX "%sÇ÷ÇÃ%s", 
			jtrns_obj('d', dungeons[mptr->br->end2.dnum].dname),
			br_string2(mptr->br));
#endif
...
*/
			Sprintf(eos(buf), " (%däK)", depth(&(mptr->br->end2)));
		if (!dump) {
			putstr(win, 0, buf);
		} else {
#ifdef DUMP_LOG
