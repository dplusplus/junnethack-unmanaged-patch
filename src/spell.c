...
#else
	    nomul(delay, "本を読んでいるときに");		/* remaining delay is uninterrupted */
#endif
	    delay = 0;
	    return(0);
	}
...
#else
		    nomul(delay, "本を読んでいるときに");			/* study time */
#endif
		    delay = 0;
		    if(gone || !rn2(3)) {
/*JP
...
*/
	dump_html("<tr><th></th><th>Name</th><th>Level</th><th>分類</th><th>成功率</th></tr>\n", "");
	for (i = 0; i < MAXSPELL && spellid(i) != NO_SPELL; i++) {
