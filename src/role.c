 ...
 */
/*JP
  ï∂éöóÒÇÃç≈èâÇÃàÍï∂éöñ⁄Çå©ÇÈÉRÅ[ÉhÇ™êèèäÇ…ë∂ç›Ç∑ÇÈÇÃÇ≈ÅA
  âpåÍñºÇécÇµÇƒÇ®Ç≠ÅB (see you.h)
*/
...
/*JP: ÉIÉvÉVÉáÉìÇ≈égÇ¡ÇƒÇ¢ÇÈÇÃÇ≈âpåÍñºÇécÇµÇƒÇ®Ç≠ÅB (see you.h) */
#if 0 /*JP*/
...
#else
{	"ïΩòaéÂã`",	"ïΩòaéÂã`é“", 	"ïΩòaìIÇ≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","ïΩòaéÂã`é“",
	"ïΩòaéÂã`é“Ç…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"â¡ãséÔñ°",	"ÉTÉfÉBÉXÉg", 	"â¡ãsìIÇ≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","ÉTÉfÉBÉXÉg",
	"ÉTÉfÉBÉXÉgÇ…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"ñ≥ê_ò_", 	"ñ≥ê_ò_é“", 	"ñ≥êMêSÇ≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","ñ≥ê_ò_é“",
	"ñ≥ê_ò_é“Ç…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"óáëÃéÂã`", 	"ÉkÅ[ÉfÉBÉXÉg", 	"óáÇ≈",		TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","ÉkÅ[ÉfÉBÉXÉg",
	"ÉkÅ[ÉfÉBÉXÉgÇ…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"ëT",		"ëTÉ}ÉXÉ^Å[", 	"ñ”ñ⁄Ç≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","ÇãÅìπÇµÇƒÇ¢ÇÈ","ÇãÅìπÇµÇƒÇ¢ÇΩ","ëTÇÃìπ",
	"ëTÇÃìπÇ©ÇÁäOÇÍÇƒ"},
#endif /*JP*/
...
#else
{	"ãÍçs",	"ãÍçsé“",	"ãÛï†Ç≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","ÇƒÇ¢Ç»Ç¢","Ç»Ç©Ç¡ÇΩ","êHéñÇÇµ",
	"ãÍçsÇ…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"åµäiÇ»çÿêHéÂã`",	"åµäiÇ»çÿêHéÂã`é“",	"åµäiÇ»çÿêHéÂã`Ç≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","åµäiÇ»çÿêHéÂã`é“",
	"åµäiÇ»çÿêHéÂã`Ç…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
#else
{	"çÿêHéÂã`", 	"çÿêHéÂã`é“",	"çÿêHéÂã`Ç≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","çÿêHéÂã`é“",
	"çÿêHéÂã`Ç…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"},
#endif /*JP*/
...
{	"illiteracy", 	"illiterate",	"illiterate",	TRUE,
	...
	"become literate"},
#else
{	"ï∂ñ”", 	"ï∂ñ”é“",	"ï∂ñ”Ç≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","ÇƒÇ¢Ç»Ç¢","Ç»Ç©Ç¡ÇΩ","ì«Ç›èëÇ´Çµ",
	"ì«Ç›èëÇ´ÇµénÇﬂÇƒ"},
#endif /*JP*/
...
#else
{	"ìêÇ›",	"ìêëØì™",	"ñÔâÓÇ≈",	TRUE,
	"Ç†Ç»ÇΩÇÕ","Ç≈Ç†ÇÈ","ÇæÇ¡ÇΩ","Ç∆ÇƒÇ‡ñÔâÓé“",
	"ìêëØì™Ç…íßêÌÇµÇƒì⁄ç¡ÇµÇƒ"}
#endif /*JP*/
};
...
*/
	static char err_ret[] = "ÉLÉÉÉâÉNÉ^ÇÃ";
...
*/
	const char *defprompt = "ìKìñÇ…ÉLÉÉÉâÉNÉ^ÇëIÇÒÇ≈ÇÊÇ¢Ç≈Ç∑Ç©ÅH[yntq] ";
...
*/
	Strcat(buf, "ÇìKìñÇ…ëIÇÒÇ≈ÇÊÇÎÇµÇ¢Ç≈Ç∑Ç©ÅH[yntq] ");
	return buf;
}
...
int nameflg;
{
    ...
    case PM_MONK:
	if(nameflg)
	    Sprintf(helo_buf, "ÉiÉ}ÉXÉeÅ[%s", plname);
	else
	    Sprintf(helo_buf, "ÉiÉ}ÉXÉeÅ[");
	break;
    case PM_SAMURAI:
	if(nameflg)
...
Goodbye(int nameflg)
{
    ...
    case PM_MONK:
	if(nameflg)
	    Sprintf(helo_buf, "Ç≥ÇÁÇŒåÂÇËÇäJÇ©ÇÒÇ∆Ç∑ÇÈ%sÇÊ", plname);
	else
	    Sprintf(helo_buf, "Ç≥ÇÁÇŒåÂÇËÇäJÇ©ÇÒÇ∆Ç∑ÇÈ");
	break;
    case PM_SAMURAI:
	if(nameflg)
...
/*JP
		You_feel("violent!");
*/
		You_feel("ñ\óÕìIÇæÇ∆ä¥Ç∂ÇΩÅI");
/*JP
		if (yn("Do you want to quit?") == 'y') {
*/
		if (yn("ÉQÅ[ÉÄÇÇ‚ÇﬂÇÈÅH") == 'y') {
		    killer_format = NO_KILLER_PREFIX;
/*JP
		    killer = "quit after an act of violence";
*/
		    killer = "ñ\óÕìIçsà◊ÇÃÇ†Ç∆î≤ÇØÇΩ";
		    done(QUIT);
		}
...
*/
		pline("ëTÇÃê∏ê_ÇÕÇ†Ç»ÇΩÇÃêgëÃÇ©ÇÁãéÇ¡ÇΩÅB");
...
*/
		You_feel("ÇŸÇÒÇÃÇÌÇ∏Ç©Ç»çﬂÇä¥Ç∂ÇΩÅB");
	    u.uconduct.food++;
	    if (u.uconduct.food >= 10) u.roleplay.ascet = FALSE;
	    break;
...
*/
		pline("äwéØÇ†ÇÈï∂ñ”é“ÇÃÇΩÇﬂÇÃêÙó˚Ç≥ÇÍÇΩï∂äwÅI");
		exercise(A_WIS, TRUE);		
		}
	    break;
...
*/
		You_feel("ï¿à»â∫ÇÃã≠ìêÇ…Ç»Ç¡ÇƒÇµÇ‹Ç¡ÇΩÇÊÇ§Ç»ãCÇ™ÇµÇΩÅB"); 
	    break;
