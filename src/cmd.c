...
#else
		Sprintf(buf, "空腹レベルは%d", u.uhunger);
		enl_msg(buf, "である", "だった", "");
#endif
#if 0 /*JP*/
...
#else
		enl_msg(conducts[cdt].failtxt, "いた", "いた", "");	/* "無謀にも平和主義者を挑んで" */
#endif
	    }
	}
...
*/
		enl_msg(You_, "していない", "さなかった", "生き物を殺");
	    } else {
#if 0 /*JP*/
		...
		you_have_X(buf);
#else
		Sprintf(buf, "%ld匹の生き物を殺し", u.uconduct.killer);
...
*/
	...
	if (wizard && u.uconduct.armoruses) {
	    ...
	    you_have_X(buf);
#else
	    Sprintf(buf, "%ld回防具を装備し", u.uconduct.armoruses);
	    enl_msg(You_, "ている", "た", buf);
#endif
	}
#endif
...
*/
		enl_msg(You_, "んでいない", "まなかった", "Elberethを刻");
	}
#endif /* ELBERETH */
