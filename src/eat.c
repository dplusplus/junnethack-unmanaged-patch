...
*/
		nomul(-rnd(10), "腐った食べ物で意識を失っている間に");
/*JP
...
*/
	...
	if ((amorphous(&mons[mnum]) || slithy(&mons[mnum]) ||
		...
		pline("Eating this %s corpse makes your %s %s slippery.",
		      ...
		      Glib ? "even more" : "very");
#else
		pline("%sしたものを食べたのであなたの%sは%s滑りやすくなった。",
		      amorphous(&mons[mnum]) ? "ぬるぬる" : "つるつる",
		      body_part(FINGER),
		      Glib ? "さらに" : "");
#endif
		incr_itimeout(&Glib, rnd(15));
...
#else
		    pline(Hallucination ? "まったりとして、それでいてしつこくない！"
		      "これぞ究極のメニューだ！" :
			  "この食べ物は本当に申し分ない！");
#endif
#if 0 /*JP*/
...
*/
		nomul(-10+(u.uhunger/10), "腹が減って倒れている間に");
/*JP
