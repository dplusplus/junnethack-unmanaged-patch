...
*/
	        pline("ÉâÉbÉLÅ[ÅIç°î”ÇÕñûåéÇæÅB");
	change_luck(1);
    } else if(flags.moonphase == NEW_MOON) {
/*JP
	pline("Be careful!  New moon tonight.");
*/
	        pline("íçà”ÇµÇÎÅIç°î”ÇÕêVåéÇæÅB");
    }
...
*/
	        pline("ópêSÇµÇÎÅIÇPÇRì˙ÇÃã‡ójì˙Ç…ÇÕÇÊÇ≠Ç»Ç¢Ç±Ç∆Ç™Ç†ÇÈÅB") ;
	change_luck(-1);
    }
...
#else
    if(new_game){
	pline("%sÅAUnNetHackÇÃê¢äEÇ÷ÅIÇ±ÇÃÉQÅ[ÉÄÇ≈ÇÕÇ†Ç»ÇΩÇÕ%sÇÃ%s(%s)ÇæÅB",
	      Hello((struct monst *) 0, TRUE), urace.j,
	      (currentgend && urole.jname.f) ? urole.jname.f : urole.jname.m,
	      buf);
    } else {
	pline("%sÅAUnNetHackÇÃê¢äEÇ÷ÅIÇ†Ç»ÇΩÇÕ%sÇÃ%sÇæÅI",
	      Hello((struct monst *) 0, TRUE), urace.j,
	      (currentgend && urole.jname.f) ? urole.jname.f : urole.jname.m);
    }
#endif
